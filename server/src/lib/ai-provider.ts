...
export type AIInput = { mode: AIMode; prompt: string; media?: AIMedia; history?: AITurn[] };
...
export function selectAIProvider(mode: AIMode, mimeType?: string): AIProvider {
  // HF's chat-completions image input is not a PDF input. PDFs have one explicit
  // Gemini route, not a second provider call or a fallback after an HF failure.
  return mode === "timetable" && mimeType !== "application/pdf" ? "huggingface" : "gemini";
}
export function providerConfiguration(env: AIEnvironment, mode: AIMode, mimeType?: string) {
  const provider = selectAIProvider(mode, mimeType);
...
export function assertAIConfiguration(env: AIEnvironment, mode: AIMode, mimeType?: string) {
  ...
  const config = providerConfiguration(env, mode, mimeType);
...
async function providerResponse(response: Response): Promise<unknown> {
  if (!response.ok) {
    // Never forward upstream response bodies: they may contain private input or keys.
...
export async function generateAI(env: AIEnvironment, input: AIInput, fetcher: typeof fetch = fetch): Promise<{ text: string; provider: AIProvider }> {
  const config = assertAIConfiguration(env, input.mode, input.media?.mimeType);
  ...
  try {
    if (config.provider === "gemini") {
      ...
      })) as { candidates?: { content?: { parts?: { text?: string; thought?: boolean }[] }; finishReason?: string }[]; promptFeedback?: { blockReason?: string } };
      ...
      const content: unknown[] = [{ type: "text", text: input.prompt || "Extract this timetable." }];
      ...
      const payload = await providerResponse(await fetcher("https://router.huggingface.co/v1/chat/completions", {
        ...
        body: JSON.stringify({ model: config.model, messages: [{ role: "system", content: system }, { role: "user", content }], max_tokens: 4096, temperature: 0.1, stream: false }),
      })) as { choices?: { message?: { content?: string }; finish_reason?: string }[] };
      const choice = payload.choices?.[0];
      if (choice?.finish_reason && choice.finish_reason !== "stop") throw new AIProviderError(502, "AI_INCOMPLETE", "The timetable response was incomplete. Import a smaller section or use a clearer image.");
