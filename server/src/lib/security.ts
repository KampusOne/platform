...
function signingKey(env: Bindings) {
  if (!env.JWT_SECRET || env.JWT_SECRET.length < 32) {
    throw new AppError(
      ...
      "Secure session signing is not configured.",
    );
  }
...
export function generateOtp(): string {
  ...
  while (random >= ceiling)
    random = crypto.getRandomValues(new Uint32Array(1))[0] ?? ceiling;
...
export async function deriveHandoffCode(
  ...
  if (!env.OTP_PEPPER || env.OTP_PEPPER.length < 24) {
    throw new AppError(
      503,
      "PROVIDER_UNAVAILABLE",
      "Secure handoff codes are not configured.",
    );
  }
  const entropy = await sha256(`${env.OTP_PEPPER}:handoff:${kind}:${orderId}`);
  const code = String(
    Number.parseInt(entropy.slice(0, 12), 16) % 1_000_000,
  ).padStart(6, "0");
...
export async function issueAccessToken(
  ...
  return new SignJWT({
    ...
    ...(user.sessionFamilyId ? { sid: user.sessionFamilyId } : {}),
  })
  ...
  try {
    ...
    return {
      ...
      roles: Array.isArray(payload.roles)
        ...
        : [],
      universityId:
        typeof payload.universityId === "string" ? payload.universityId : null,
      operatorRoles: Array.isArray(payload.operatorRoles)
        ? payload.operatorRoles.filter(
            (role): role is string => typeof role === "string",
          )
        : [],
    };
