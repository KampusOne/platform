...
describe("conversation API boundaries", () => {
  for (const [method, path] of [
    ["PUT", `/v1/student/feed/${P1}/view`],
    ["POST", `/v1/student/feed/${P1}/comments`],
    ["GET", `/v1/admin/public-badges/${U1}`],
    ["PUT", `/v1/admin/public-badges/${U1}`],
  ]) {
...
describe("conversation migration on an isolated PostgreSQL-compatible database", () => {
  ...
  it("is safe to rerun and preserves existing replies", async () => {
    await db.exec(migration);
    expect((await db.query<{ body: string }>("select body from public.feed_comments where body='Existing reply stays intact'")).rows).toEqual([{ body: "Existing reply stays intact" }]);
  });
  it("permits text, image-only and combined replies", async () => {
    ...
    expect((await db.query<{ total: number }>("select count(*)::int as total from public.feed_comments where media_object_id=$1", [M1])).rows[0]?.total).toBe(2);
  });
  ...
  it("deduplicates repeated views and binds the view to the correct post campus", async () => {
    ...
    expect((await db.query<{ total: number }>("select count(*)::int as total from public.feed_post_views")).rows[0]?.total).toBe(2);
    ...
    expect((await db.query<{ relrowsecurity: boolean }>("select relrowsecurity from pg_class where oid='public.feed_post_views'::regclass")).rows[0]?.relrowsecurity).toBe(true);
  });
  it("assigns and removes a public badge without changing identity verification", async () => {
    ...
    const result = await db.query<{ verification_status: string; badge: boolean }>("select verification_status,coalesce((to_jsonb(p)->>'public_badge_verified')::boolean,verification_status='VERIFIED',false) as badge from public.profiles p order by user_id");
