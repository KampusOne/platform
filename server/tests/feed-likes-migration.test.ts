...
beforeAll(async () => {
  ...
  await db.exec(`
    ...
    create table public.feed_posts(id uuid primary key, university_id uuid not null references public.universities(id), status text not null, published_at timestamptz);
    insert into public.users values('${alice}'),('${bob}');
    insert into public.universities values('${campus}'),('${otherCampus}');
    insert into public.feed_posts values
