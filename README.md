...
Phase 3 now includes a dark, rehearsed vendor-operations slice: revision-safe storefront setup, product moderation and stock controls, detailed fulfilment, administrator review queues and explicit delivery-zone policy entry. A separately gated, read-only demo catalogue supports Store browsing and cart testing without persisted vendor records or checkout. Production Store, Logistics, Marketplace and Payments remain disabled while checkout quote, managed media, delivery enforcement and finance controls are completed. See the [Phase 3 commerce foundation handoff](docs/phases/phase-3-commerce-foundation.md).
