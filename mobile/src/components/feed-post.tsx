...
export const FeedPost = memo(function FeedPost({ post, onBookmark, onShare, onDeleted, onFeedback, onChanged, onComment, detail = false }: {
  ...
  const canCollapse = !detail && (body.length > collapsedCharacters || text.paragraphs.length > 3);
  ...
  const copy = text.title || body ? (
    <View style={styles.postCopy}>
      {text.title ? (
        <RichPostText style={[styles.postTitle, detail && styles.detailText]}>
          {text.title}
        </RichPostText>
      ) : null}
      {body ? (
        <RichPostText
          ...
          style={[styles.postText, detail && styles.detailText]}
        >
          {body}
        </RichPostText>
      ) : null}
    ...
    <View style={styles.contentRegion}>
      <View style={styles.postContent}>
        ...
        <Pressable accessibilityRole={detail ? undefined : "button"} accessibilityLabel={`Open conversation by ${post.source_name}`} onPress={openPost} style={[styles.postBody, detail && styles.detailBody]}>
          ...
          {post.image_url ? post.media_type?.startsWith("video/") ? <MediaPreview url={post.image_url} video label="Post video" watermark={post.source_name} /> : <MediaImage accessible accessibilityIgnoresInvertColors accessibilityLabel="Post attachment" resizeMode="contain" uri={post.image_url} style={styles.postImage} /> : null}
...
const createStyles = (theme: Theme) => StyleSheet.create({
  post: { borderBottomColor: theme.border, borderBottomWidth: StyleSheet.hairlineWidth, paddingTop: 10, paddingBottom: 3 },
  contentRegion: { position: "relative" },
  postContent: { width: "100%" },
  postHeader: { flexDirection: "row", alignItems: "flex-start", gap: 9, paddingRight: 36 },
  sourceCopy: { flex: 1, minWidth: 0, paddingTop: 1 },
  sourceNameRow: { flexDirection: "row", alignItems: "center", gap: 4, minHeight: 22 },
  sourceName: { flexShrink: 1, color: theme.text, fontFamily: theme.font.semibold, fontSize: 14 },
  postTime: { color: theme.textMuted, fontFamily: theme.font.body, fontSize: 11.5, flexShrink: 0 },
  menu: { position: "absolute", top: -5, right: -5 },
  postBody: { paddingLeft: 47, marginTop: 0, minHeight: 26 },
  detailBody: { paddingLeft: 0, marginTop: 12 },
  postCopy: { gap: 4 },
  postTitle: { color: theme.text, fontFamily: theme.font.semibold, fontSize: 14, lineHeight: 20 },
  postText: { color: theme.text, fontFamily: theme.font.body, fontSize: 14, lineHeight: 20 },
  detailText: { fontSize: 16, lineHeight: 23 },
  seeMore: { alignSelf: "flex-start", color: theme.deepBrand, fontFamily: theme.font.semibold, fontSize: 13, lineHeight: 20, marginTop: 2 },
  structuredPanel: { backgroundColor: theme.surfaceMuted, borderColor: theme.border, borderRadius: 12, borderWidth: StyleSheet.hairlineWidth, marginTop: 3, padding: 10 },
  structuredEyebrow: { color: theme.brandPressed, fontFamily: theme.font.bold, fontSize: 9, letterSpacing: 0.7, marginBottom: 4 },
  urgent: { color: theme.statusAttention, fontFamily: theme.font.semibold, fontSize: 11, marginTop: 7 },
  postImage: { width: "100%", aspectRatio: 1.4, borderRadius: 12, marginTop: 8 },
  correction: { flexDirection: "row", alignItems: "flex-start", gap: 7, backgroundColor: theme.surfaceMuted, borderRadius: 10, marginTop: 8, padding: 9 },
  correctionText: { color: theme.statusAttention, flex: 1, fontFamily: theme.font.medium, fontSize: 11, lineHeight: 16 },
  actions: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginLeft: 47, minHeight: 44, flexWrap: "wrap" },
  fullActions: { marginLeft: 0 },
  action: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 4, minHeight: 44, minWidth: 44 },
  actionText: { color: theme.textMuted, fontFamily: theme.font.medium, fontSize: 11 },
  pressed: { opacity: 0.7 },
  detailMeta: { flexDirection: "row", flexWrap: "wrap", alignItems: "center", gap: 8, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: theme.border, paddingVertical: 3, marginTop: 10 },
  exactTime: { color: theme.textMuted, fontFamily: theme.font.body, fontSize: 11 },
  sponsored: { color: theme.textSubtle, fontFamily: theme.font.bold, fontSize: 8, letterSpacing: 0.8, marginLeft: 47 },
  repostedBy: { flexDirection: "row", gap: 5, marginLeft: 47, marginBottom: 7, alignItems: "center" },
  repostedText: { color: theme.textMuted, fontFamily: theme.font.medium, fontSize: 11, flex: 1 },
});
