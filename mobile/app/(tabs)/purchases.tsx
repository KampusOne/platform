...
type DisputeTarget = {
  resourceType: "TUTORIAL_BOOKING" | "STORE_ORDER";
  id: string;
  title: string;
};
...
const ORDER_DISPUTE_STATUSES = new Set([
  "PAID",
  "ACCEPTED",
  "READY",
  "IN_DELIVERY",
...
function BookingRecord({
  ...
  const canCancel =
    ...
    Date.parse(item.scheduled_for!) > currentTime;
  ...
  const completionAvailable =
    hasValidCompletionTime && completionAvailableTime <= currentTime;
  ...
  return (
    <View
      style={[
        styles.record,
        first && styles.recordFirst,
        last && styles.recordLast,
        !last && styles.recordDivider,
      ]}
    >
      ...
      <View style={styles.detailLine}>
        <Ionicons
          name={item.scheduled_for ? "calendar-outline" : "receipt-outline"}
          size={15}
          color={theme.textMuted}
        />
        <Text style={styles.detailText}>
          {item.scheduled_for
            ? `Scheduled for ${dateTime(item.scheduled_for)}`
            : `Booked ${date(item.created_at)}`}
        </Text>
      </View>
      ...
      {item.status === "CONFIRMED" && !completionAvailable ? (
        <View
          accessible
          accessibilityLabel={completionHint}
      ...
      {item.review_id ? (
        <View style={styles.reviewedRow}>
          ...
          <Text style={styles.reviewedText}>
            You rated this tutorial {item.review_rating}/5
          </Text>
        </View>
      ) : null}
    </View>
    ...
    >
      ...
      <View style={styles.orderBreakdown}>
        <Text style={styles.breakdownLabel}>
          Items {naira(item.subtotal_kobo)}
        </Text>
        <View style={styles.breakdownDot} />
        <Text style={styles.breakdownLabel}>
          Delivery {naira(item.delivery_fee_kobo)}
        </Text>
      </View>
      ...
      {item.delivery_code ? (
        <View
          accessible
          accessibilityLabel={`Delivery code ${item.delivery_code}`}
          style={styles.deliveryCode}
        >
          <View style={styles.deliveryCopy}>
            <Text style={styles.deliveryLabel}>DELIVERY CODE</Text>
            <Text style={styles.deliveryHelp}>
              Share only when your order arrives.
            </Text>
          </View>
          <Text selectable style={styles.deliveryValue}>
            {item.delivery_code}
          </Text>
        </View>
      ) : null}
  ...
  return (
    <View style={styles.emptyState}>
      ...
      <Text style={styles.emptyBody}>
        Your tutorial bookings and campus store orders will be kept here after
        you make them.
      </Text>
      <Pressable
  ...
  return (
    <View style={styles.sectionEmpty}>
      <View style={styles.sectionEmptyIcon}>
        <Ionicons
          name={booking ? "calendar-outline" : "bag-handle-outline"}
          size={22}
          color={theme.brandPressed}
        />
      </View>
      <View style={styles.sectionEmptyCopy}>
        <Text style={styles.sectionEmptyTitle}>
          No {booking ? "tutorial bookings" : "store orders"}
        </Text>
        <Text style={styles.sectionEmptyBody}>
          New {booking ? "bookings" : "orders"} will appear here.
        </Text>
      </View>
    </View>
  );
...
function LoadingPurchases() {
  ...
  return (
    <View
      accessibilityLabel="Loading bookings and orders"
...
export default function PurchasesScreen() {
  ...
  const [completionTarget, setCompletionTarget] = useState<Booking | null>(
    null,
  );
  ...
  useEffect(() => {
    const now = Date.now();
    const nextCompletionTime = data.tutorialBookings.reduce(
      (next, booking) => {
        if (booking.status !== "CONFIRMED" || !booking.completion_available_at)
          return next;
        const completionAvailableTime = Date.parse(
          booking.completion_available_at,
        );
        if (
          !Number.isFinite(completionAvailableTime) ||
          completionAvailableTime <= now
        )
          return next;
        return next === null || completionAvailableTime < next
          ? completionAvailableTime
          : next;
      },
      null as number | null,
    );
  ...
  async function pay(
    resourceType: DisputeTarget["resourceType"],
    resourceId: string,
  ) {
    setBusy(resourceId);
    setError("");
  ...
  async function confirmBooking(id: string) {
    ...
    try {
      const result = await api<{ status: string }>(
        `/v1/student/tutorial-bookings/${id}/confirm`,
        {
          method: "POST",
  ...
  async function cancelBooking(id: string) {
    ...
    try {
      ...
      }>(`/v1/student/tutorial-bookings/${id}/cancel`, {
        method: "POST",
  ...
  async function openDispute() {
    ...
    try {
      ...
      setNotice(
        "Your dispute is open and the related earnings are frozen for review.",
      );
      setDispute(null);
      setReason("");
      await load();
  ...
  const hasPurchases =
    data.tutorialBookings.length > 0 || data.orders.length > 0;
  ...
  const completionBusy = Boolean(
    completionTarget && busy === completionTarget.id,
  );
  ...
  return (
    <SafeAreaView edges={["top"]} style={styles.safe}>
      ...
      <SectionList<PurchaseItem, PurchaseSection>
        ...
        keyExtractor={(item) =>
          `${"course_code" in item ? "booking" : "order"}-${item.id}`
        }
            ...
            <View style={styles.header}>
              ...
              <View style={styles.headerCopy}>
                ...
                <Text style={styles.headerSubtitle}>
                  Bookings and campus orders
                </Text>
              </View>
              <Pressable
                accessibilityLabel={
                  loading ? "Refreshing purchases" : "Refresh purchases"
                }
                accessibilityRole="button"
                ...
                style={({ pressed }) => [
                  styles.headerButton,
                  loading && styles.secondaryDisabled,
        ...
        renderSectionFooter={({ section }) => (
          <View>
            {section.data.length === 0 ? (
              <SectionEmpty kind={section.kind} />
            ) : null}
            <View style={styles.sectionGap} />
          </View>
        )}
        renderSectionHeader={({ section }) => (
          <SectionHeading count={section.count} title={section.title} />
        )}
        sections={sections}
      ...
      >
        <View style={styles.modalBackdrop}>
          ...
          >
            ...
            <Text style={styles.completionTarget}>
              {completionTarget?.course_code} · {completionTarget?.title}
            </Text>
            <Text style={styles.completionBody}>
              Only confirm if the tutorial was delivered. This is final. Once
              the tutor also confirms, it can move their earnings toward payout
              and start the 48-hour dispute period.
            </Text>
            ...
            <View style={styles.completionActions}>
              <Pressable
                ...
                onPress={() => setCompletionTarget(null)}
                style={({ pressed }) => [
                  styles.completionCancel,
                  completionBusy && styles.secondaryDisabled,
                  pressed && !completionBusy && styles.pressed,
                ]}
              >
              ...
              <Pressable
                ...
                onPress={() => {
                  if (completionTarget)
                    void confirmBooking(completionTarget.id);
                }}
                style={({ pressed }) => [
                  styles.completionConfirm,
                  completionBusy && styles.actionDisabled,
                  pressed && !completionBusy && styles.pressed,
                ]}
              >
                {completionBusy ? (
      ...
      >
        ...
        >
          <Pressable
            accessibilityLabel="Close tutorial review"
            accessibilityRole="button"
            disabled={reviewBusy}
            ...
            >
              <View style={styles.modalTop}>
                <View style={styles.modalHeadingCopy}>
                  ...
                  <Text numberOfLines={1} style={styles.modalTarget}>
                    {reviewTarget?.course_code} · {reviewTarget?.title}
                  </Text>
                </View>
                <Pressable
              ...
              <Text style={styles.modalBody}>
                Your rating is tied to a completed booking, which helps other
                students judge the session fairly.
              </Text>
              ...
              />
              {error ? (
                <Text accessibilityRole="alert" style={styles.formReviewError}>
                  {error}
                </Text>
              ) : null}
              <Pressable
      ...
      >
        ...
        >
          ...
          <View
            accessibilityViewIsModal
            onAccessibilityEscape={() => setDispute(null)}
          ...
          >
            ...
            >
              ...
              <Text style={styles.modalBody}>
                Tell support what happened. The related earnings will be frozen
              ...
              <View style={styles.inputHeader}>
                ...
                <Text
                  style={[
                    styles.inputCount,
                    reasonLength >= 10 && styles.inputCountValid,
                  ]}
                >
                  {reasonLength >= 10
                    ? `${reasonLength} characters`
                    : `${reasonLength}/10 minimum`}
                </Text>
              </View>
              <TextInput
              ...
              <Text style={styles.inputHelp}>
                Include what you expected, what happened, and any relevant time
                or handoff detail.
              </Text>
              ...
              <Pressable
                ...
                style={({ pressed }) => [
                  styles.submitAction,
                  disputeDisabled && styles.actionDisabled,
                  pressed && !disputeDisabled && styles.pressed,
                ]}
              >
                {disputeBusy ? (
                  <ActivityIndicator color="#FFFFFF" size="small" />
...
const createStyles = (theme: Theme) =>
  StyleSheet.create({
    ...
    notice: {
      ...
      backgroundColor: "rgba(75,123,84,0.09)",
      borderRadius: 14,
      flexDirection: "row",
      gap: 9,
      marginBottom: 14,
    ...
    bookingMarker: {
      alignItems: "center",
      backgroundColor: "rgba(233,177,142,0.24)",
      borderRadius: 13,
      height: 44,
      justifyContent: "center",
      width: 44,
    ...
    illustrationHalo: {
      backgroundColor: "rgba(241,223,200,0.60)",
      borderRadius: 62,
      bottom: 2,
      height: 122,
      left: 34,
    ...
    sectionEmptyIcon: {
      alignItems: "center",
      backgroundColor: "rgba(233,177,142,0.22)",
      borderRadius: 13,
      height: 44,
      justifyContent: "center",
      width: 44,
    ...
    errorIcon: {
      alignItems: "center",
      backgroundColor: "rgba(168,70,46,0.09)",
      borderRadius: 24,
      height: 66,
      justifyContent: "center",
      width: 66,
    ...
    sheetHandle: {
      alignSelf: "center",
      backgroundColor: "rgba(41,35,31,0.16)",
      borderRadius: 2,
      height: 4,
      marginBottom: 14,
      width: 38,
