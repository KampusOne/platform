...
export default function ProfileScreen() {
  ...
  const purchasesKnown =
    purchasesState === "ready" || purchasesState === "stale";
  ...
  async function uploadPhoto() {
    ...
    setUploading(true);
    try {
      const file = await pickAndUpload("avatar");
      if (file) {
        setProfile((p) => (p ? { ...p, profile_image_url: file.url } : p));
        await reloadProfile();
  ...
  return (
    <ProductScreen style={styles.screen}>
        ...
        <View style={styles.identity}>
          ...
          >
            ...
            {profile?.faculty_name ? (
              <Meta icon="school-outline" text={profile.faculty_name} />
            ) : null}
            <Meta
              icon="business-outline"
              text={profile?.university_name ?? "University not set"}
            />
            {profile?.graduation_year ? (
              <Meta
                icon="calendar-outline"
                text={`Graduating ${profile.graduation_year}`}
              />
            ) : null}
          </ScrollView>
        </View>
      ...
      {dataNotice ? (
        <View style={styles.notice}>
          <InlineFeedback message={dataNotice} />
        </View>
      ) : null}
      <AgentShortcuts />
      ...
      {(profile || sessionProfile) ? (
        <>
          <View style={styles.metrics}>
            <Metric
              ...
              value={
                hideCgpa
                  ? "Hidden"
                  : academicsKnown && academics?.summary?.cgpa
                    ? String(academics.summary.cgpa)
                    : "—"
              }
            />
            <Metric
            ...
            <Metric
              ...
              label="Recent saves"
              value={feedKnown ? String(bookmarks.length) : "—"}
            />
          </View>
            ...
            <>
              <View style={styles.section}>
                ...
                {feedState === "idle" ? <ListSkeleton count={2} /> : !feedKnown ? (
                  ...
                  <Text style={styles.quietState}>
                    Recent posts you bookmark in Feed will appear here.
                  </Text>
                )}
              </View>
          ...
          {activeTab === "Classes" ? (
            <View style={styles.tabPanel}>
              <EmptyResult
                ...
                body="Your current classes, venues and reminders live in Timetable Manager."
                icon="calendar-outline"
                onAction={() => router.push("/timetable")}
                title="Your class history"
              />
            </View>
          ) : null}
          {activeTab === "Transactions" ? (
            <View style={styles.tabPanel}>
              <EmptyResult
                ...
                body={
                  purchasesKnown
                    ? "Track tutorial bookings, store orders, delivery codes and payment status."
                    : "Transaction totals are unavailable right now. Open Purchases to retry."
                }
          ...
          <Pressable
            ...
            onPress={() => void Linking.openURL("https://agents.kampusone.app")}
            style={({ pressed }) => [
              styles.agentLink,
              pressed && styles.pressed,
            ]}
          >
            ...
            <View style={styles.agentCopy}>
              <Text style={styles.agentTitle}>Become a KampusOne agent</Text>
              <Text style={styles.agentBody}>
                Apply as a tutor, vendor or campus rider.
              </Text>
            </View>
      ...
      <AccountMenu
        ...
        onSelect={(item) => {
          ...
          else if (item === "Help & support") router.push("/support");
          else if (item === "Account manager") router.push("/account");
        }}
...
function Meta({
  ...
  return (
    <View style={styles.meta}>
      <Ionicons color={theme.deepBrand} name={icon} size={16} />
        ...
        >
          {items.map(([label, icon], index) => (
            <Pressable
              ...
              style={[
                styles.menuItem,
                index === items.length - 1 && styles.menuLogout,
              ]}
            >
              <Ionicons
                color={
                  index === items.length - 1 ? theme.deepBrand : theme.text
                }
                name={icon}
                size={20}
              />
              <Text
                style={[
                  styles.menuText,
                  index === items.length - 1 && styles.menuLogoutText,
                ]}
              >
                {label}
              </Text>
...
const createStyles = (theme: Theme) =>
  StyleSheet.create({
    ...
    coverShade: {
      backgroundColor: "rgba(41,35,31,0.08)",
      bottom: 0,
      left: 0,
      position: "absolute",
      right: 0,
