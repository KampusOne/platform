...
function TutorCard({ tutor }: { tutor: TutorSummary }) {
  ...
  return (
    ...
    >
      ...
      <Text numberOfLines={1} style={styles.tutorCardName}>
        {tutor.name}
      </Text>
...
function resourceMeta(resource: Resource) {
  ...
  return resource.access_model === "FREE"
    ? "Free preview"
    : "Access with tutorial";
}
    ...
    >
      ...
      <View style={styles.resourceCopy}>
        ...
        <Text style={styles.resourceDetail}>
          {resource.level_code ?? resource.batch_label ?? "Campus resource"} ·{" "}
      ...
      const url = access.media_object_id
        ? (
            await api<{ url: string }>(
              `/v1/media/${access.media_object_id}/access`,
              { method: "POST" },
            )
          ).url
        : access.file_url;
      if (!url || !url.startsWith("https://"))
        throw new Error("This resource has no downloadable file.");
      await Linking.openURL(url);
    } catch (e) {
      toast(
        e instanceof Error ? e.message : "Could not open resource.",
        "error",
      );
    } finally {
      setOpening(false);
  ...
  return (
    ...
    >
      <View style={styles.previewBackdrop}>
        <Pressable
          accessibilityLabel="Close resource preview"
          accessibilityRole="button"
          onPress={onClose}
        ...
        <View accessibilityViewIsModal style={styles.previewSheet}>
          ...
          >
            ...
            <Text style={styles.previewText}>
              {resource?.preview_text ?? "A preview has not been added yet."}
            </Text>
          </ScrollView>
          {resource?.file_url || resource?.media_object_id ? (
            <Pressable
            ...
            <View style={styles.previewNotice}>
              <Ionicons
                ...
                size={18}
              />
              <Text style={styles.previewNoticeText}>Preview only</Text>
            </View>
          )}
        </View>
      </View>
...
function TutorialCard({
  ...
  const spaces = selected
    ? Math.max(0, selected.capacity - selected.booked_spaces)
    : 0;
  ...
  return (
    <View style={styles.listingCard}>
        ...
        <View style={styles.listingCopy}>
          <View style={styles.listingMetaRow}>
            ...
            <Text style={styles.price}>
              {naira(Number(listing.price_kobo))}
            </Text>
          </View>
      ...
      <Text numberOfLines={3} style={styles.listingDescription}>
        {listing.description}
      </Text>

      <View style={styles.listingTrustRow}>
        <Ionicons color={theme.statusAttention} name="star" size={14} />
        <Text style={styles.listingTrustText}>
          {Number(listing.review_count) > 0
            ? `${Number(listing.rating).toFixed(1)} · ${listing.review_count} reviews`
            : "New tutorial"}
        </Text>
        <View style={styles.trustDot} />
        <Text style={styles.listingTrustText}>
          {listing.location_text ?? displayFormat(listing.format)}
        </Text>
      </View>
      ...
      <View style={styles.timeHeader}>
        ...
        <Text style={styles.spaces}>
          {selected
            ? `${spaces} ${spaces === 1 ? "space" : "spaces"} left`
            : "No times yet"}
        </Text>
      </View>
          ...
          {listing.availability.map((window) => {
            ...
            return (
              <Pressable
                ...
                style={({ pressed }) => [
                  styles.window,
                  active && styles.windowSelected,
                  pressed && styles.pressed,
                ]}
              >
                <Ionicons
                  color={active ? "#FFFFFF" : theme.brandPressed}
                ...
                <Text
                  style={[
                    styles.windowText,
                    active && styles.windowTextSelected,
                  ]}
                >
                  {formatWindow(window)}
                </Text>
  ...
  return (
    <View style={styles.emptyState}>
      ...
      <Text style={styles.emptyTitle}>
        {filtered ? "No tutorials match" : "No tutorials published yet"}
      </Text>
...
export default function TutorialsScreen() {
  ...
  const formats = useMemo(() => {
    const availableFormats = Array.from(
      new Set(listings.map((listing) => displayFormat(listing.format))),
    );
    return ["All", ...availableFormats];
  }, [listings]);
  const activeFormat = formats.includes(selectedFormat)
    ? selectedFormat
    : "All";
  ...
  const filtered = useMemo(
    () =>
      listings.filter((item) => {
        ...
        const matchesQuery =
          !needle ||
          `${item.course_code} ${item.title} ${item.description} ${item.tutor_name}`
            .toLowerCase()
            .includes(needle);
        const matchesFormat =
  ...
  const filteredResources = useMemo(
    () =>
      resources.filter((item) => {
        ...
        const matchesQuery =
          !needle ||
          `${item.course_code} ${item.title} ${item.description} ${item.publisher_name}`
            .toLowerCase()
            .includes(needle);
        const matchesCategory =
          selectedCategory === "All" ||
          resourceCategory[item.resource_type] === selectedCategory;
  ...
  const showListings =
    selectedCategory === "All" || selectedCategory === "Tutors";
      ...
      if (existing) {
        if (!existing.courses.includes(listing.course_code)) {
          tutorsById.set(tutorId, {
            ...existing,
            courses: [...existing.courses, listing.course_code],
          });
        }
        continue;
      }
  ...
  const book = useCallback(
    async (listing: Listing) => {
      ...
      try {
        ...
        const availabilityWindowId = listing.availability.some(
          (window) => window.id === rememberedWindowId,
        )
          ? rememberedWindowId
        ...
        if (booking.status === "CONFIRMED" || booking.amountKobo === 0) {
          const message =
            "Your free tutorial is booked. Open Purchases to see the session details.";
          setNotice(message);
        ...
        if (savedBookingId) {
          ...
          const message =
            caught instanceof ApiError
              ? caught.message
              : "The tutorial could not be booked.";
  ...
  const hasActiveFilter =
    ...
    selectedCategory !== "All";
  const canShowEmpty =
    Boolean(listings.length || resources.length) || (!loading && !loadError);
  ...
  return (
    <SafeAreaView edges={["top"]} style={styles.screen}>
      <FlatList
        ...
        ListEmptyComponent={
          canShowEmpty && !filteredResources.length ? (
            <TutorialsEmptyState filtered={hasActiveFilter} />
          ) : null
        }
        ListHeaderComponent={
          <>
            <SearchField
              ...
              placeholder="Search a course, topic or tutor"
              value={query}
            />
                ...
                <View style={styles.sectionHeader}>
                  <View>
                    ...
                    <Text style={styles.sectionSubtitle}>
                      People teaching the sessions below
                    </Text>
                  </View>
                  <View style={styles.approvedKey}>
                    ...
                    />
                    <Text
                      accessibilityLabel={`${tutors.length} tutors`}
                      style={styles.approvedKeyText}
                    >
                  ...
                  <View style={styles.resourceSection}>
                    <View style={styles.availableHeader}>
                      <View>
                        ...
                        <Text style={styles.sectionSubtitle}>
                          Notes, questions, PDFs and audio
                        </Text>
                      </View>
                    ...
                    <View style={styles.resourceList}>
                      {filteredResources.map((resource) => (
                        <ResourceRow
                          key={resource.id}
                          onOpen={() => setPreviewResource(resource)}
                          resource={resource}
                        />
                      ))}
                    </View>
...
const createStyles = (theme: Theme) =>
  StyleSheet.create({
    ...
    formatChip: {
      backgroundColor: "rgba(241,223,200,.62)",
      borderRadius: 8,
      paddingHorizontal: 7,
      paddingVertical: 4,
    },
    ...
    trustDot: {
      backgroundColor: "rgba(41,35,31,.24)",
      borderRadius: 2,
      height: 3,
      marginHorizontal: 2,
      width: 3,
    ...
    bookButtonPressed: { opacity: 0.84, transform: [{ scale: 0.97 }] },
    bookButtonText: {
      color: "#FFFFFF",
      fontFamily: theme.font.bold,
      fontSize: 11.5,
    ...
    resourceRowPressed: {
      backgroundColor: "rgba(241,223,200,.28)",
      opacity: 0.82,
    },
    resourceIcon: {
      alignItems: "center",
    ...
    previewBackdrop: {
      backgroundColor: "rgba(41,35,31,.42)",
      flex: 1,
