...
export default function PostScreen() {
  ...
  const [focusToken, setFocusToken] = useState(comments === "1" ? 1 : 0);
  ...
  function openComments() { setFocusToken((value) => value + 1); scroller.current?.scrollTo({ y: commentY.current, animated: false }); }
  ...
  return <SafeAreaView style={styles.screen}>
    ...
    <KeyboardAvoidingView style={styles.screen} behavior={Platform.OS === "ios" ? "padding" : Platform.OS === "android" ? "height" : undefined}>
      <ScrollView ref={scroller} keyboardShouldPersistTaps="handled" contentContainerStyle={styles.content}>
        {!valid ? <View style={styles.state}><Text style={styles.title}>Post unavailable</Text><Text style={styles.body}>This post link is not valid.</Text></View>
          ...
          : post ? <>
              <FeedPost post={post} onBookmark={(value) => void bookmark(value)} onShare={(value) => void share(value)} onFeedback={setFeedback} onComment={openComments}
              ...
              {post.social_enabled ? <View onLayout={(event) => { commentY.current = event.nativeEvent.layout.y; }}><CommentThread key={`${key}:comments`} postId={post.id} focusToken={focusToken} onUpdated={refreshPost} /></View> : null}
            </>
...
const createStyles = (theme: Theme) => StyleSheet.create({
  screen: { flex: 1, backgroundColor: theme.canvas }, header: { flexDirection: "row", alignItems: "center", gap: 8, paddingHorizontal: 16, width: "100%", maxWidth: 540, alignSelf: "center" },
  back: { minWidth: 44, minHeight: 48, justifyContent: "center", alignItems: "center" }, heading: { color: theme.text, fontFamily: theme.font.semibold, fontSize: 18 },
  content: { padding: 20, paddingBottom: 48, width: "100%", maxWidth: 540, alignSelf: "center" }, state: { alignItems: "center", paddingVertical: 44, gap: 14 },
  title: { color: theme.text, fontFamily: theme.font.display, fontSize: 23, textAlign: "center" }, body: { color: theme.textMuted, fontFamily: theme.font.body, fontSize: 14, lineHeight: 21, textAlign: "center" },
  button: { minHeight: 48, paddingHorizontal: 22, paddingVertical: 14, borderRadius: 14, backgroundColor: theme.deepBrand, marginTop: 8 }, buttonText: { color: "#FFFFFF", fontFamily: theme.font.semibold, fontSize: 14 },
