...
type Status = { enabled: boolean; historyDays: number; providers: { study: { configured: boolean; missing: string[] } }; allowance: { remaining: number; limit: number; resetsAt: string; globalAvailable: boolean; policy: string } };
...
export default function StudyAI() {
  ...
  return (
    <ToolPage title="Study space">
      ...
      <Text style={{ ...mutedStyle, marginBottom: 12 }}>A new provider attempt uses one allowance, including failed attempts. Checking an existing attempt or reopening saved work does not.</Text>
