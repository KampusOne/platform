...
export default function StudentAI() {
  ...
  const sheetPanel=<View style={{flex:1,justifyContent:'flex-end',backgroundColor:'rgba(0,0,0,0.35)'}}><Pressable accessibilityLabel="Close panel" accessibilityRole="button" onPress={()=>setSheet(null)} style={{flex:1}}/>
    ...
    <KeyboardAvoidingView behavior={Platform.OS==='ios'?'padding':undefined} style={{flex:1,width:'100%',maxWidth:760,alignSelf:'center'}}>
      ...
      <ScrollView ref={scroll} style={{flex:1}} contentContainerStyle={{flexGrow:1,paddingHorizontal:24,paddingBottom:16}} keyboardShouldPersistTaps="handled" keyboardDismissMode="interactive" onContentSizeChange={()=>{if(pending)scroll.current?.scrollToEnd({animated:false});}}>
        ...
        {loaded&&draftOwner.current===user?.id?turns.map(turn=>{
          ...
          return <View key={turn.requestId}>{renderUser(turn.prompt??'',file)}<Text style={{...muted,fontFami{Platform.OS==='web'
      ? (sheet!==null?<View style={[StyleSheet.absoluteFillObject,{zIndex:100000,elevation:100}]}>{sheetPanel}</View>:null)
      : <Modal visible={sheet!==null} transparent animationType="fade" onRequestClose={()=>setSheet(null)}>{sheetPanel}</Modal>}>
